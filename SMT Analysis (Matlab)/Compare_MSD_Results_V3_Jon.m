...
startlocn = {'C:\Users\pgomez.CRG\Desktop\Temporal'};
...
edges_D_density_plot = {0:5:300 0:5:300}; %This is for the density plot of radius of gyration vs radius of confinement.


%-----------------------------------------------------------------
%For the Density plot (Max Rad Gyration vs Radius of Confinement) analysis---------------------------------------------------
%---------------------------------------------------------------------
Radius_Conf_threshold_density = 0.3; %Radius of confinement threhold in order to filter the tracks for the analysis of the density plots (Max rad gyration vs Radius of confinement). 
%Only tracks with Radius of confienent smaller than this values (in um)    
%will be used.
Num_points_diff = 5; %Number of nearest neighbor from H2B Data to calculate the difference between TFs and H2B. The H2B value will be the mean of those points.
...
%   Cell_centerOfMass_MaxDist_Area{t} = struct2cell(load(strcat(files_CenterOfMass{t}.data{1,2},files_CenterOfMass{t}.data{1,1})));
...
%Fit a Normal distribution to each condition
 %------------------------------------------------------------------
 %Test Bimodality of the distribution:
probability_unimodal_Radius(t) = test_bimodality(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000));
...
pStart = 0.4;
muStart = quantile(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000),[0.25 0.75]);
muStart = [1.2 1.8];
sigmaStart = sqrt(var(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000)) - 0.25*diff(muStart).^2);
start = [pStart muStart sigmaStart sigmaStart];

lb = [0 0 0 0 0];
ub = [1 10 10 Inf Inf];
%%%pdf_normmixture = @(x,p,mu1,mu2,sigma1,sigma2)p*normpdf(x,mu1,sigma1) + (1-p)*normpdf(x,mu2,sigma2)
paramEsts_radius{t} = mle(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000), 'pdf',pdf_normmixture, 'start',start,'lower',lb, 'upper',ub);

xgrid = linspace(0.9*min(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000)),1.1*max(log10(f_confined_circ_diff_R_all{t}(iidx{t})*1000)),200);
...
figure()
for t=1:ntypes;
iidx_Jon{t} = find(f_confined_circ_diff_R_all{t} <= Radius_Conf_threshold_density);
     
bin_size = 2;
binCtrs = 0:bin_size:160; %Bin centers. (depends on your data)
n=length(f_confined_circ_diff_R_all{t}(iidx_Jon{t}));
counts = hist((f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000),binCtrs);
...
%Fit a Normal distribution to each condition
 %------------------------------------------------------------------
 %Test Bimodality of the distribution:
probability_unimodal_Radius(t) = test_bimodality(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000);
%  
% %Fit a Normal distribution
% 
% % if probability_unimodal(t) > 0.9;
...
pStart = 0.4;
muStart = [27 42];
sigmaStart = sqrt(var(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000) - 0.25*diff(muStart).^2);
start = [pStart muStart sigmaStart sigmaStart];
...
paramEsts_radius{t} = mle(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000, 'pdf',pdf_normmixture, 'start',start,'lower',lb, 'upper',ub,'options',options);

xgrid = linspace(0.9*min(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000),1.1*max(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000),200);
pdfgrid_radius{t} = pdf_normmixture(xgrid,paramEsts_radius{t}(1),paramEsts_radius{t}(2),paramEsts_radius{t}(3),paramEsts_radius{t}(4),paramEsts_radius{t}(5));
hold on
plot(xgrid,pdfgrid_radius{t},'-','LineWidth',2,'Color',Color(t,:));
...
%Fit a Normal distribution to each condition
 ...
 %Test Bimodality of the distribution:
iidx_Jon{t} = find(f_confined_circ_diff_R_all{t} <= Radius_Conf_threshold_density);
probability_unimodal_Radius(t) = test_bimodality(f_confined_circ_diff_R_all{t}(iidx{t})*1000);
                  
pStart1 = 0.4;
pStart2 = 0.3;
muStart = quantile(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000,[0.25 0.5 0.75]);
muStart = [27 42 70];
sigmaStart = sqrt(var(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000) - 0.25*diff(muStart).^2);
start = [pStart1 pStart2 muStart sigmaStart(1) sigmaStart(1) sigmaStart(1)];

lb = [0 0 20 40 65 0 0 0];
ub = [1 1 30 50  75 Inf Inf Inf];
pdf_normmixture_3 = @(x,p1,p2,mu1,mu2,mu3,sigma1,sigma2,sigma3)p1*normpdf(x,mu1,sigma1) + p2*normpdf(x,mu2,sigma2) + (1-p1-p2)*normpdf(x,mu3,sigma3);
cauchy_mixture_2 = @(x,p,mu1,mu2,sigma1,sigma2)(p/pi)*(sigma1/(((x-mu1)^2) + sigma1^2)) + ((1-p)/pi)*(sigma2/(((x-mu2)^2) + sigma2^2));
options = statset('MaxIter',300, 'MaxFunEvals',600);
paramEsts_radius{t} = mle(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000, 'pdf',pdf_normmixture_3, 'start',start,'lower',lb, 'upper',ub,'options',options);

xgrid = linspace(0.9*min(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000),1.1*max(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000),200);
pdfgrid_radius{t} = pdf_normmixture_3(xgrid,paramEsts_radius{t}(1),paramEsts_radius{t}(2),paramEsts_radius{t}(3),paramEsts_radius{t}(4),paramEsts_radius{t}(5),paramEsts_radius{t}(6),paramEsts_radius{t}(7),paramEsts_radius{t}(8));
hold on
plot(xgrid,pdfgrid_radius{t},'-','LineWidth',2,'Color',Color(t,:));
...
lb = [0 0 20 40 65 0 0 0];
ub = [1 1 30 50  75 Inf Inf Inf];
cauchy_mixture_3 = @(x,p1,p2,mu1,mu2,mu3,sigma1,sigma2,sigma3)(p1/pi).*(sigma1./(((x-mu1).^2) + sigma1.^2)) + ((p2)./pi).*(sigma2./(((x-mu2).^2) + sigma2.^2)) + ((1-p2-p1)./pi).*(sigma3./(((x-mu3).^2) + sigma3.^2));
options = statset('MaxIter',300, 'MaxFunEvals',600);
paramEsts_radius{t} = mle(f_confined_circ_diff_R_all{t}(iidx_Jon{t})*1000, 'pdf',cauchy_mixture_3, 'start',start,'lower',lb, 'upper',ub,'options',options);
...
figure()
plotTracks_Color(Trajectories_Low_R{t},Color_R(1,:),1);
hold on
plotTracks_Color(Trajectories_Mid_R{t},Color_R(2,:),1);
hold on
plotTracks_Color(Trajectories_High_R{t},Color_R(3,:),1);
hold on
MSD_Results{t}.ma.labelPlotTracks;

jo=legend('Low R', 'Mid R', 'High R');
set(jo, 'Interpreter', 'none');
end
...
%   for i=1:size(Cell_centerOfMass_MaxDist_Area{t}{1},2);
%       
%       %Calculate the size of each cell (um)
% %       Cell_radius{t}{i} = sqrt(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4)/pi); %Assuming that the cell is a circle, calculate its size in order to normalize the distance to the center.
%       Cell_radius{t}{i} = Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3)/2; %Assuming that the cell is a circle, calculate its size based on the Half Maximum distance
% 
%       
...
%           
%           Track_Distance_to_CellCenter_High_R{t}{i}(j) = (sqrt( (Tracks_CenterOfMass_High_R{t}{i}(j,1) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(1))^2 + (Tracks_CenterOfMass_High_R{t}{i}(j,2) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(2))^2))/Cell_radius{t}{i};
% 
%           end
%       
%         
...
%           
%           Track_Distance_to_CellCenter_Mid_R{t}{i}(j) = (sqrt( (Tracks_CenterOfMass_Mid_R{t}{i}(j,1) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(1))^2 + (Tracks_CenterOfMass_Mid_R{t}{i}(j,2) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(2))^2))/Cell_radius{t}{i};
% 
%           end
%       
%         
...
%           
%           Track_Distance_to_CellCenter_Low_R{t}{i}(j) = (sqrt( (Tracks_CenterOfMass_Low_R{t}{i}(j,1) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(1))^2 + (Tracks_CenterOfMass_Low_R{t}{i}(j,2) - Cell_centerOfMass_MaxDist_Area{t}{1}{i}(2))^2))/Cell_radius{t}{i};
% 
%           end
%       
%         
...
%          
%         bin_size = 0.1;
%         binCtrs = 0:bin_size:1.5; %Bin centers. (depends on your data)
...
% 
%         xlabel('Normalized Distance','FontSize',20,'FontWeight','bold');
%         ylabel('Frequency','FontSize',20,'FontWeight','bold');
%         title(strcat('Histogram of Track distance to the center of the cell - ',files{t}.dataname(1:end-21)),'FontSize',24,'FontWeight','bold');
...
%       end
%     
% %       Performing NND Normalization with Random distribution of points in
% %       the same area
%         if size(Tracks_Cell_High_R{t}{i},1)>2;
%         NND_High_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_High_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_High_R{t}{i},1),100,NND_High_R{t}{i});
%         end
%         NND_High_Random2Real_mean_Ratio{t} = nonzeros(NND_High_Random2Real_mean_Ratio{t});
% 
%         
%   end
%   
...
%     
%     strOut = sprintf('Calculating & Normalizing Nearest Neighbor Distances: % 4.1f',100*t/(ntypes));
%     fprintf([strB strOut '%%\n']);
%     strB = repmat('\b',1,length(strOut)+2);
% 
% end
...
%       % Performing NND Normalization with Random distribution of points in
% %       the same area
%         if size(Tracks_Cell_Mid_R{t}{i},1)>2;
%         NND_Mid_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_High_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_Mid_R{t}{i},1),100,NND_Mid_R{t}{i});
%         end
%         NND_Mid_Random2Real_mean_Ratio{t} = nonzeros(NND_Mid_Random2Real_mean_Ratio{t});
%       
%       
%   end
%     
...
%          % Performing NND Normalization with Random distribution of points in
% %       the same area
%         if size(Tracks_Cell_Low_R{t}{i},1)>2;
%         NND_Low_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_High_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_Low_R{t}{i},1),100,NND_Low_R{t}{i});
%         end
%         NND_Low_Random2Real_mean_Ratio{t} = nonzeros(NND_Low_Random2Real_mean_Ratio{t});
%       
%       
%   end
%     
...
%    figure()         
%         bin_size = 0.08;
%         binCtrs = 0:bin_size:1.2; %Bin centers. (depends on your data)
%         n=length(NND_High_Random2Real_mean_Ratio{t});
%         counts = hist(NND_High_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(1,:),'FaceAlpha',0.6);
%         hold on;
%        
%         n=length(NND_Mid_Random2Real_mean_Ratio{t});
%         counts = hist(NND_Mid_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(2,:),'FaceAlpha',0.6);
%         hold on;
%         
%          n=length(NND_Low_Random2Real_mean_Ratio{t});
%         counts = hist(NND_Low_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(3,:),'FaceAlpha',0.6);
% 
%         xlabel('NND ratio','FontSize',20,'FontWeight','bold');
%         ylabel('Frequency','FontSize',20,'FontWeight','bold');
%         title(strcat('Histogram of NND Random to Real Ratio',files{t}.dataname(1:end-21)),'FontSize',24,'FontWeight','bold');
%         legend('High-High Radius Conf','Medium-Medium Radius Conf','Low-Low Radius Conf');
%         set(gca,'FontSize',20,'FontWeight','bold');  
%         xlim([-0.1 1.3]);
%      
% end
...
%   for i=1:size(Cell_centerOfMass_MaxDist_Area{t}{1},2);
%       
%       if size(Tracks_Cell_High_R{t}{i},1) > 0;
%       
%       for j = 1:size(Tracks_Cell_High_R{t}{i},1);
%           if size(Tracks_Cell_Mid_R{t}{i},1)>1;
...
% %       the same area
%         if size(Tracks_Cell_Mid_R{t}{i},1)>2;
%         NND_High_Mid_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_High_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_Mid_R{t}{i},1)+1,10,NND_High_Mid_R{t}{i});
%         end
%         NND_High_Mid_Random2Real_mean_Ratio{t} = nonzeros(NND_High_Mid_Random2Real_mean_Ratio{t});
%       
%       
%   end
%   
...
%         % the same area
%         if size(Tracks_Cell_Low_R{t}{i},1)>2;
%         NND_High_Low_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_High_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_Low_R{t}{i},1)+1,10,NND_High_Low_R{t}{i});
%         end
%         NND_High_Low_Random2Real_mean_Ratio{t} = nonzeros(NND_High_Low_Random2Real_mean_Ratio{t});   
%       
%   end
%     
...
%       
%       
%       if size(Tracks_Cell_Mid_R{t}{i},1) > 0;
%       
%       for j = 1:size(Tracks_Cell_Mid_R{t}{i},1);
%           if size(Tracks_Cell_Low_R{t}{i},1)>1;
...
%         % the same area
%         if size(Tracks_Cell_Low_R{t}{i},1)>2;
%         NND_Mid_Low_Random2Real_mean_Ratio{t}(i) = NND_normalization_V2(Cell_centerOfMass_MaxDist_Area{t}{1}{i}(4),Tracks_CenterOfMass_Mid_R{t}{i},Cell_centerOfMass_MaxDist_Area{t}{1}{i}(3),size(Tracks_Cell_Low_R{t}{i},1)+1,10,NND_Mid_Low_R{t}{i});
%         end
%         NND_Mid_Low_Random2Real_mean_Ratio{t} = nonzeros(NND_Mid_Low_Random2Real_mean_Ratio{t});   
%       
%       
%   end
%     
...
%         hold on;
%        
%         n=length(NND_High_Mid_Random2Real_mean_Ratio{t});
%         counts = hist(NND_High_Mid_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(2,:),'FaceAlpha',0.6);
%         hold on;
%         
%          n=length(NND_High_Low_Random2Real_mean_Ratio{t});
%         counts = hist(NND_High_Low_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(3,:),'FaceAlpha',0.6);
...
%         ylabel('Frequency','FontSize',20,'FontWeight','bold');
%         title(strcat('Histogram of NND Random to Real Ratio',files{t}.dataname(1:end-21)),'FontSize',24,'FontWeight','bold');
%         legend('High-High Radius Conf','High-Medium Radius Conf','High-Low Radius Conf');
%         set(gca,'FontSize',20,'FontWeight','bold');  
%         xlim([-0.1 1.6]);
%         
% end
...
%    figure()         
%         bin_size = 0.1;
%         binCtrs = 0:bin_size:1.7; %Bin centers. (depends on your data)
%         
%         n=length(NND_Low_Random2Real_mean_Ratio{t});
%         counts = hist(NND_Low_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
...
%         hold on;
%        
%         n=length(NND_Mid_Low_Random2Real_mean_Ratio{t});
%         counts = hist(NND_Mid_Low_Random2Real_mean_Ratio{t},binCtrs); %In nm
% 
%         prob = counts / (n*bin_size);
%         H = bar(binCtrs,prob,'hist');
%         set(H,'Facecolor',Color_R(2,:),'FaceAlpha',0.6);
...
%         ylabel('Frequency','FontSize',20,'FontWeight','bold');
%         title(strcat('Histogram of NND Random to Real Ratio',files{t}.dataname(1:end-21)),'FontSize',24,'FontWeight','bold');
%         legend('Low-Low Radius Conf','Low-Medium Radius Conf','Low-High Radius Conf');
%         set(gca,'FontSize',20,'FontWeight','bold');  
%         xlim([-0.1 1.8]);
%      
% end
...
for t=1:ntypes;  
idx_Jon{t} = find(f_confined_circ_diff_R_all{t} <= Radius_Conf_threshold_density); %Take only those tracks with R smaller than 300nm
figure()
out{t} = scatplot(f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000,Max_gyration_all{t}(idx_Jon{t})*1000,'circles',0.032,100,5,3,8);
title(strcat(files{t}.dataname(1:end-9)),'FontSize',24,'FontWeight','bold','Interpreter','None'); 
set(gca,'FontSize',20,'FontWeight','bold');  
xlabel('Radius of confinement (nm)');
ylabel('Max Radius of gyration (nm)');
axis('square');
xlim([0 80]);
ylim([0 80]);
end
...
for t=1:ntypes;  
figure()
scatter(f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000,Plateau_value_gyration_all{t}(idx_Jon{t})*1000,'MarkerEdgeAlpha',0.1);
xlim([0 80]);
ylim([0 80]);
end


%% Compare ALL the density scatter plots to H2B, using H2B as a reference. (Make sure that type.t1 is H2B)

Reference_density_H2B = (out{1}.dd)/max(out{1}.dd); 
Reference_position_H2B = [f_confined_circ_diff_R_all{1}(idx_Jon{1})*1000 ; Max_gyration_all{1}(idx_Jon{1})*1000]';
strB = '';
for t=2:ntypes;
TF_density = (out{t}.dd)/max(out{t}.dd);
TF_position = [f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000 ; Max_gyration_all{t}(idx_Jon{t})*1000]';    
    

        for i=1:size(TF_position,1);
        dist_idx{t}{i} = knnsearch(Reference_position_H2B,TF_position(i,:),'K',Num_points_diff);        
        Difference_percentage{t}(i) = (mean(-Reference_density_H2B(dist_idx{t}{i})) + TF_density(i))/mean(Reference_density_H2B(dist_idx{t}{i}));   
        end
...
x = f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000;
y = Max_gyration_all{t}(idx_Jon{t})*1000;
for k = 1:size(map,1);
    if any(ind==k);
   line('Xdata',x(ind==k),'Ydata',y(ind==k),'LineStyle','none','Color',map(k,:),'Marker','.','MarkerSize',4);
    end
end

title(strcat('Differences between H2B & ',files{t}.dataname(1:end-9)),'FontSize',24,'FontWeight','bold','Interpreter','None'); 
set(gca,'FontSize',20,'FontWeight','bold');  
xlabel('Radius of confinement (nm)');
ylabel('Max Radius of gyration (nm)');
axis('square');
...
xlim([0 80]);
ylim([0 80]);
caxis([min(Difference_percentage{t}) max(Difference_percentage{t})]);
...
%% ddd
% %Find the center of mass of the subpopulations
% for t=1:ntypes;
%     
% Max_dd{t} = max(out{t}.dd);
% Min_dd{t} = min(out{t}.dd);
% end
% 
% %Perform density based clustering
% for t=1:ntypes; 
% rng('default');
% idx_Jon{t} = find(f_confined_circ_diff_R_all{t} <= 0.1); %Take only those tracks with R smaller than 300nm
% figure()
% X_db = [f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000; Max_gyration_all{t}(idx_Jon{t})*1000];
% 
% SimMatrix=pdist2(X_db,X_db,'minkowski',2);
% Tclass=Mdbscan(X_db,50,1,SimMatrix)';  
% 
% if size(X_db,2)==2
...
%     set(gcf,'color','w');
% else
%     Matrix=pdist2(X_db,X_db,'minkowski',2);    %% plot MDS if ndata has more than 2 attributes
...
% X_db = X_db';
% 
% k=ceil(size(X_db,1)^0.5);
% SimMatrix= SNN(X_db,k);
...
% for t=1:ntypes;  
% figure()
% densityplot(f_confined_circ_diff_R_all{t}(iidx{t})*1000,ma{t}.lfit.a(iidx{t})*1000000/2/ma{t}.n_dim,edges_D_density_plot);
% title(strcat(files{t}.dataname(1:end-9)),'FontSize',24,'FontWeight','bold','Interpreter','None'); 
% set(gca,'FontSize',20,'FontWeight','bold');  
% xlabel('Radius of confinement (nm)');
% ylabel('Diffusion Coefficient (um2/s)');
% axis('square');
% xlim([0 100]);
% ylim([0 400]);
% end
...
% for t=1:ntypes;  
% figure()
% scatter(f_confined_circ_diff_R_all{t}*1000,Half_life_gyration_all{t},'MarkerEdgeAlpha',0.1);
% axis('square')
% end
%  

 
%% Detect clusters in the Rad.Gyration vs Rad.Confinement plot with kmeans
% rng default; % For reproducibility
% X = [f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000; Max_gyration_all{t}(idx_Jon{t})*1000]';
...
%% Detect clusters in the Rad.Gyration vs Rad.Confinement plot with Estimation-Maximization (EM) Method.
% rng(3);
% k = 5;
% Sigma = 'full';
...
% d = 500;
% for t=1:ntypes;
% idx_Jon{t} = find(f_confined_circ_diff_R_all{t} <= 0.3); %Take only those tracks with R smaller than 300nm
% end
% X = [f_confined_circ_diff_R_all{t}(idx_Jon{t})*1000; Max_gyration_all{t}(idx_Jon{t})*1000]';
...
% 
% figure()
% gmfit = fitgmdist(X,k,'CovarianceType',Sigma,'SharedCovariance',SharedCovariance,'Options',options);
