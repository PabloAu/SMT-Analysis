...
SimulatedTracks = 0; %If the track file is a .mat file from TrackGenerator_Model.m.

Input_space_units = 'micron'; %Specify the input spatial units of the Tracks (With Trackmate I usually use microns)
Input_time_units = 'frames'; %Specify the input time units of the Tracks ('frames' or 'second') (With TrackMate I usually use Frames)
...
Detect_butterfly = 1; %Binary decision for whether or not to detect the butterfly trajectories. 
...
In_percentage = 0.7; %Normalized percentage of points of a trayectory inside a certain subROI in order to be considered an "INSIDE" track.
...
pixel_size = 0.160;  %Effective Pixel size in µm
...
Track_segment_interval = []; %Specify the frames that will be used for the analysis of the trajectories. Ex: [1 10] takes only the first 10 frames of each trajectory. 
%Leave it empty [] for using the hole trajectory.
...
%------------------------------------------------------------------
level = 0.005; %This is the minimum MSD value. It depens on the localization precision. level = 4*(Loc_precition^2) [in µm]. 
...
[list_names,directory_name] = uigetfile(fullfile(directory_name,'*.mat'),'select the files with SMT Simulated Tracks from TrackGenerator','MultiSelect','on');    
  
if iscell(list_names);
        strB = '';
        for g=1:size(list_names,2);
        Simulated_file = load(fullfile(strcat(directory_name,'\'),list_names{g}));  
        trayectory{g} = Simulated_file.tracks';
        tracklength{g} = cellfun('length',trayectory{g});
        strOut = sprintf('Loading and extracting the Data: % 4.1f',100*g/(size(list_names,2)));
            fprintf([strB strOut '%%\n']);
            strB = repmat('\b',1,length(strOut)+2);
        end
    else
    Simulated_file = load(fullfile(strcat(directory_name,'\'),list_names));  
    trayectory{1} = Simulated_file.tracks';
    tracklength{1} = cellfun('length',trayectory{1});
    end 
...
[trayectories_INSIDE, trayectories_OUTSIDE,list_in] = tracksINROI(trayectory_filtered,sROI,pixel_size,image_names,subROI_names,In_percentage);
...
%% Take only the fragments of the tracks specified by "Track_segment_interval"
if ~isempty(Track_segment_interval);
    
    trayectories_FrameInt_idx = cellfun(@(x) find(size(x,1) >= Track_segment_interval(2)), trayectories,'UniformOutput',0);
    trayectories_FrameInt_idx_logi = ~cellfun('isempty', trayectories_FrameInt_idx);
    trayectories = cellfun(@(x) x(Track_segment_interval(1):Track_segment_interval(2),:), trayectories(find(trayectories_FrameInt_idx_logi == 1)),'UniformOutput',0);
    
    fprintf('\n');    
    fprintf('---------------------------------');
    fprintf('\n');    
    fprintf('Taking only the frame interval of the tracks:\n');
    fprintf(strcat('[',num2str(Track_segment_interval(1)),'-',num2str(Track_segment_interval(2)),']'));
    fprintf('\n');
    fprintf('---------------------------------');
    fprintf('\n');
...
if Detect_butterfly == 1;
save(fullfile(strcat(directory_name,'\MSD_Results_',num2str(minimum_track_length),'min_frames','_subROIs',num2str(subROIs),'_Inside',num2str(Inside)),strcat('msd_results_',num2str(Frame_interval),'ms','_Butterfly_segments_reference_ID','.mat')),'Butterfly_Reference_Track_segment');
end
